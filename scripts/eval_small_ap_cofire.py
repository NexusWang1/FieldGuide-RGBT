#!/usr/bin/env python
"""eval_small_ap_cofire.py — eval_small_ap.py + 烟雾"同帧有火/无火"子集分析

与 eval_small_ap.py 的唯一差异：新增两个子集箱
  - smoke_cofire ：同帧存在 fire GT 的图像中的 smoke GT
  - smoke_nofire ：同帧不存在 fire GT 的图像中的 smoke GT
用于检验 FieldGuide 的增益是否集中于"有火可寻"子集（机制预测：是），
以及无火子集是否退化（边界声明）。

口径与 small/large 分箱一致：箱外 GT 被移除、落在移除 GT 上的预测记为 FP，
两模型同口径横向对比有效。

服务器运行（conda YOLO26，cwd=/data/wwc/install/ultralytics）：

    python eval_small_ap_cofire.py \
        --weights runs/phy-rgb/<run>/weights/best.pt \
        --data /data/wwc/test/RGBT-3M/rgbt_full.yaml --mode rgbt --split val --device 0

结果 JSON 写入 <weights目录>/eval_cofire_<split>_<mode>.json
（文件名与 eval_small_ap.py 不同，不会覆盖旧结果）
"""
...
BINS = ("all", "small", "large", "smoke_cofire", "smoke_nofire")
...
def collect_image_stats(pred, gt_cls, gt_box_norm, img_hw, stats_bins):
    ...
    is_smoke = gt_cls == SMOKE
    has_fire = bool((gt_cls == FIRE).any())
    fire_mask = np.full(len(gt_cls), has_fire, dtype=bool)  # 避免 ~python_bool 变整数的坑

    bins = {
        "all": np.ones(len(gt_cls), dtype=bool),
        "small": areas < SMALL_AREA,
        "large": areas >= SMALL_AREA,
        # 新增：smoke 实例按同帧有无 fire 拆子集（非 smoke 的 GT 不进箱）
        "smoke_cofire": is_smoke & fire_mask,
        "smoke_nofire": is_smoke & ~fire_mask,
    }
    ...
    # 帧计数（子集规模佐证）
    if (is_smoke & fire_mask).any():
        stats_bins["smoke_cofire"]["n_img"] += 1
    if (is_smoke & ~fire_mask).any():
        stats_bins["smoke_nofire"]["n_img"] += 1
...
def main():
    ...
    out_path = Path(args.weights).parent.parent / f"eval_cofire_{args.split}_{args.mode}.json"
    ...
    print(fmt_table(results["smoke_cofire"], "SMOKE subset: 同帧有火"))
    print(fmt_table(results["smoke_nofire"], "SMOKE subset: 同帧无火"))
    print(f"\n[eval] saved -> {out_path}")
